  ...
  <div>
      ...
      <div class="card-head">
        <a-row>
          <a-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
            <b style="color: #08632d"> ADMINISTRACIÓN DE USUARIOS SOCIO </b>
          </a-col>
        </a-row>
      </div>
    </a-card>
    ...
    <a-modal v-model="visibleModalRegitro" :title="tituloModal" :dialog-style="{ top: '20px' }" :width="700">
      ...
      <a-form-model ref="ruleForm" :model="form" :rules="rules" size="small">
        ...
        <a-row>
          <a-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
            <a-form-model-item label="Socio :" :label-col="formItemTowLayout.labelCol"
              :wrapper-col="formItemTowLayout.wrapperCol">
              <a-select v-model="form.recaudadorId" :disabled="dissabled">
                <a-select-option v-for="(recaudador, index) in recaudadorasList" :key="index" v-bind:value="recaudador.recaudadorId">
                  {{ recaudador.nombre }}
                </a-select-option>
              </a-select>
            </a-form-model-item>
...
import Sucursal from "../../../service/Administraciones/Sucursal.service";
...
export default {
  ...
  watch: {
    'form.recaudadorId'(value) {
      Sucursal.getLstSucursalRecaudadorForAddUserByRecaudadorId(value).then(r => {
  ...
  methods: {
    ...
    findPersonas(page) {
      ...
      AdminUsuarioSocioEmpresa.findPersonasByTipo(this.formBusqueda, 82)
        .then((response) => {
    ...
    procesarOperacion() {
      ...
      switch (this.transaccion) {
        case "CREAR":
          if (this.form.recaudadorId == null) {
    ...
    savePersona() {
      ...
      AdminUsuarioSocioEmpresa.savePersonaSocioEmpresa(82, this.form)
        .then((response) => {
          this.findPersonas(1);
          this.visibleModalRegitro = false;
    ...
    updatePersonaSocioEmpresa() {
      this.form.modulo = this.modulo;
      AdminUsuarioSocioEmpresa.updatePersonaSocioEmpresa(82, this.form)
        .then((response) => {
